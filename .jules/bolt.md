...
## 2026-04-22 - Eliminate Data Fetching Waterfalls in Next.js Pages
**Learning:** Sequential `await` calls that do not depend on each other cause unnecessary Time To First Byte (TTFB) delays. In `src/app/[lang]/(shop)/shop/page.tsx`, the products query unnecessarily waited for the categories query to resolve when no category filter was applied.
**Action:** When multiple independent queries run on a page, launch them simultaneously (e.g., as floating promises or via `Promise.all`) and `await` them only when their data is required. Ensure any un-awaited background promises have a dummy `.catch(() => {})` attached to prevent unhandled rejection crashes.
