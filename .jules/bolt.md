...
## 2026-04-24 - Unnecessary useSession renders in UI containers
**Learning:** The Next-Auth `useSession()` hook forces components to re-render when auth state changes. When imported into generic layout containers (like `Header.tsx`) but unused, it causes the entire app shell to pointlessly re-render on auth changes.
**Action:** Audit layout and container components to ensure `useSession()` is only called in leaf components or wrapper components that actually require the auth state to conditionally render UI.
