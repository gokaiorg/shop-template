...
## 2024-05-25 - Deduplicate Next.js data fetching with React.cache()
**Learning:** In Next.js App Router, while native `fetch` requests are automatically deduplicated during a single request lifecycle (e.g., between `generateMetadata` and the page component), SDK calls like `firebase-admin` database queries are NOT. Without deduplication, these SDK calls will execute twice, hitting the database multiple times per page load.
**Action:** When extracting database queries that are shared between `generateMetadata` and the page component, always import `cache` from `"react"` and wrap the data fetching function (e.g., `const getProduct = cache(async (id) => { ... })`). This memoizes the result for the duration of the request, eliminating duplicate database operations.
