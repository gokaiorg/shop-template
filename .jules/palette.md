...
## 2025-04-28 - Icon Replacements in Forms
**Learning:** Hardcoded strings like "Show" and "Hide" within UI components that otherwise accept localization dictionaries break internationalization and take up unnecessary space.
**Action:** Always prefer standard icons (e.g., `lucide-react`) equipped with `aria-hidden="true"` over raw text labels for simple toggles, placing the explicit `aria-label` on the interactive parent element instead.
