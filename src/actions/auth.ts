...
export async function updateProfile(uid: string, data: { name?: string, email?: string, password?: string }) {
    ...
    // Explicit authorization check: ensure the authenticated user matches the target uid
    // or has an 'admin' role to prevent Insecure Direct Object Reference (IDOR).
    if (!session?.user || (session.user.id !== uid && session.user.role?.toLowerCase() !== 'admin')) {
